...
class PNumber:
    ...
    def __add__(self, other):
        if self.base != other.base:
            raise ValueError("Основания должны совпадать")
        return self.from_decimal(self.to_decimal() + other.to_decimal())
    ...
    def __truediv__(self, other):
        ...
        return self.from_decimal(self.to_decimal() / b)
    
    def sqr(self):
        return self.from_decimal(self.to_decimal() ** 2)
    
    def rev(self):
        d = self.to_decimal()
        if abs(d) < 1e-15:
            raise ZeroDivisionError("Деление на ноль")
        return self.from_decimal(1.0 / d)
...
class CalculatorWindow(QMainWindow):
    def __init__(self):
        ...
        self.accumulator = None          # первый операнд (накопленный результат)
        self.pending_operation = None     # ожидающая бинарная операция
        self.last_operation = None        # последняя выполненная бинарная операция
        self.last_second_operand = None   # второй операнд последней бинарной операции
        self.waiting_for_operand = True   # ожидание ввода второго операнда
        self.memory = PNumber('0', base=10, mode='real')
        self.memory_active = False
        self.history = []
        self.base = 10
        self.mode = 'real'
        self.just_calculated = False      # флаг, что только что получили результат
    ...
    def handle_operation(self, op):
        ...
        if self.accumulator is None:
            ...
            # Уже есть первый операнд
            if not self.waiting_for_operand:
                # Введён второй операнд – сначала выполняем предыдущую операцию, если она есть
                if self.pending_operation is not None:
                    self.calculate()
                # Теперь начинаем новую операцию с текущим числом в качестве первого операнда
                self.accumulator = self.current_number
                self.pending_operation = op
                self.last_operation = op
                self.waiting_for_operand = True
                ...
                # Ждём второй операнд – просто меняем операцию
                self.pending_operation = op
                self.last_operation = op
                self.just_calculated = False
    ...
    def sqr_function(self):
        try:
            ...
            if self.accumulator is None:
                ...
                # Это второй операнд – после функции он готов к вычислению
                self.waiting_for_operand = False
    ...
    def calculate(self):
        ...
        if self.accumulator is not None and self.pending_operation is not None:
            try:
                ...
                # Запоминаем последнюю операцию и второй операнд для повтора
                self.last_operation = self.pending_operation
                self.last_second_operand = second
                self.accumulator = result
                self.current_number = result
    ...
    def equals(self):
        ...
        if self.pending_operation is not None:
            # Есть незавершённая операция – выполняем её
            self.calculate()
        elif self.last_operation is not None and self.last_second_operand is not None:
            # Повтор последней операции
            try:
                second = self.last_second_operand
                ...
                self.add_to_history(f"{self.accumulator} {self.last_operation} {second} = {result}")
                self.accumulator = result
                self.current_number = result
                # last_operation и last_second_operand не меняем
                self.just_calculated = True
                self.waiting_for_operand = True
                self.update_display()
    ...
    def copy_to_clipboard(self):
        QApplication.clipboard().setText(str(self.current_number))
    ...
    def change_base(self, new_base):
        ...
        if self.last_second_operand is not None:
            self.last_second_operand = self.last_second_operand.set_base(new_base)
        if self.memory_active:
